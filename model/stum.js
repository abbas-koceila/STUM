...
export const getRdvPasse = async (id_Utilisateur) => {
  let connexion = await promesseConnexion;

  try {
    const RdvPasse = await connexion.all(
      `SELECT * FROM rendez_vous RV INNER JOIN urgence U ON RV.id_utilisateur = U.id_utilisateur WHERE etat_urgence = 0 AND U.id_utilisateur = ?`,
      [id_Utilisateur]
    );
    console.log(RdvPasse)
    return RdvPasse;
  }
  catch (error) {
    console.log(error);
...
export const getRdvFutur = async (id_Utilisateur) => {
  ...
  try {
    const RdvFutur = await connexion.all(
